          ...
          {user && (
            <div className="absolute bottom-4 right-4 hidden gap-1 max-[899px]:flex max-[899px]:bottom-2 max-[899px]:right-2">
              ...
              >
                <HeartIcon
                  className="h-5 w-5"
                  fill={saveSuccess ? "currentColor" : "none"}
                />
              </button>
            </div>
          )}
        ...
        <div className="flex h-full flex-col max-[899px]:text-center">
          <div className="mb-3 mt-3 flex flex-col gap-3.5">
            <p className="text-[0.75rem] uppercase tracking-[2px]">
              {ocrData?.isPromo ? "Versión Especial" : card.set?.name} ·{" "}
              {card.collector_number}{" "}
              {card.collector_count ? ` / ${card.collector_count}` : ""}
            </p>
              ...
              {user && (
                <span className="flex gap-1 max-[899px]:hidden">
                  <button
                    onClick={onTranslate}
                    disabled={isTranslating}
                    className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--surface-soft)]"
                  >
                    {isTranslating ? (
                  ...
                  <button
                    onClick={onSave}
                    disabled={isSaving || !!saveSuccess}
                    className={`flex h-7 w-7 items-center justify-center rounded-full ${saveSuccess ? "bg-green-500 text-white" : "bg-[var(--surface-soft)]"}`}
                  >
                    <HeartIcon
                      className="h-4 w-4"
                      fill={saveSuccess ? "currentColor" : "none"}
