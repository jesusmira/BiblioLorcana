      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 lg:px-8 lg:py-24">
        ...
        <div className="mt-20">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {inks.map((ink) => (
              <div key={ink.name} className="group relative rounded-2xl border border-[var(--stroke)]/30 bg-[var(--panel)]/40 p-6 backdrop-blur-sm transition-all duration-300 hover:border-[var(--accent)]/40 hover:bg-[var(--panel)]/60">
                <div className="flex flex-col items-center text-center">
                  <div className={clsx("mb-4 flex h-12 w-12 items-center justify-center rounded-xl border border-[var(--stroke)]/30 bg-[var(--panel)]", ink.color)}>
                    <ink.icon className="h-6 w-6 text-white" />
                  </div>
        ...
        <div className="mt-24">
          ...
          <div className="space-y-12">
            {starterDecks.map((set) => (
              <div key={set.set} className="rounded-2xl border border-[var(--stroke)]/30 bg-[var(--panel)]/40 p-6 backdrop-blur-sm">
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {set.decks.map((deck, idx) => (
                    <div key={idx} className="group relative rounded-xl border border-[var(--stroke)]/30 bg-[var(--panel)]/60 p-4 transition-all hover:border-[var(--accent)]/40">
                      <div className="flex items-center gap-4">
                        <div className="relative w-44 shrink-0">
                          <div className="aspect-[3/4] overflow-hidden rounded-lg border-2 border-[var(--accent)]/30 shadow-xl">
                            <Image
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-2">
                            {deck.inks.map((ink) => (
                              <span key={ink} className="rounded-full px-3 py-1 text-xs font-medium bg-[var(--accent)]/20 text-[var(--accent)]">
                                {ink}
                              </span>
                            ))}
                          </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {decks.map((deck) => (
                <Link key={deck.id} href={`/guias-mazos/${deck.id}`}>
                  <div className="group relative rounded-2xl border border-[var(--stroke)]/30 bg-[var(--panel)]/40 p-6 backdrop-blur-sm transition-all duration-300 hover:border-[var(--accent)]/40 hover:bg-[var(--panel)]/60 cursor-pointer h-full">
                    <div className="flex items-start gap-6 h-full">
                      <div className="relative w-48 shrink-0">
                        <div className="aspect-[3/4] overflow-hidden rounded-xl border-2 border-[var(--accent)]/30 shadow-xl shadow-[var(--accent)]/10">
                          <Image
          ...
          <div className="grid items-center gap-12 lg:grid-cols-2">
              ...
              <ul className="mt-8 space-y-4">
                {tips.map((tip) => (
                  <li key={tip} className="flex items-start gap-4 text-[var(--muted)]">
                    <span className="mt-1.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--accent)]/20">
            ...
            <div className="relative">
              <div className="relative mx-auto max-w-sm">
                ...
                <div className="absolute -right-8 top-8 -z-10 h-full w-full rotate-6 overflow-hidden rounded-2xl border border-[var(--stroke)]/30 bg-[var(--panel)]/50 opacity-70">
                  <Image
                ...
                <div className="absolute -left-8 top-8 -z-20 h-full w-full -rotate-6 overflow-hidden rounded-2xl border border-[var(--stroke)]/30 bg-[var(--panel)]/50 opacity-50">
                  <Image
